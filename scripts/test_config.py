...
def test_config_loads():
    ...
    _ok(f"deployment    = {cfg.deployment}")
    _ok(f"temperature   = {cfg.temperature}")
    _ok(f"top_p         = {cfg.top_p}")
    _ok(f"MCP servers   = {[s.name for s in cfg.mcp.servers]}")
    _ok(f"version       = {cfg.metadata.version}")
    return cfg
